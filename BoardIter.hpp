...
void Board::iterateMoves(Game& game, const MoveBoard& moveBoards, U64 piecesWithoutCards, bool player, U32 cardStuff, U8 sideCard, const bool createPiece, const U32 passTrough) const {
	...
	U32 bitScan = (piecesWithoutCards >> (player ? 32 : 0)) & MASK_PIECES;
	...
	while (_BitScanForward(&fromI, bitScan)) {
		...
		U64 newPiecesWithoutLandPiece = piecesWithoutCards & ~(((U64)fromBit) << (player ? 32 : 0));
		for (int i = 0; i <= createPiece && reverse; i++) { // spawn a piece, simulate taking for reverse moves
			...
			if (reverse)
				scan &= ~(piecesWithoutCards | _rotr64(piecesWithoutCards, 32)); // blocked by any occupied square
			else
				scan &= ~(piecesWithoutCards >> (player ? 32 : 0));	// blocked by own pieces, opponent pieces are taken
			while (scan) {
