...
void Board::iterateMoves(const GameCards& gameCards, const MoveBoard& moveBoards, U64 piecesWithNewCards, bool player, uint8_t maxDepth) const {
	//card.print();
...
template<MoveFunc cb>
void Board::forwardMoves(const GameCards& gameCards, uint8_t maxDepth) const {
...
template<MoveFunc cb>
void Board::reverseMoves(const GameCards& gameCards, uint8_t maxDepth) const {
	bool player = !(pieces & MASK_TURN);
	const CardsPos& cardsPos = CARDS_LUT[(pieces & MASK_CARDS) >> INDEX_CARDS];
